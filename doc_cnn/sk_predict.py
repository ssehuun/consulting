...
tf.flags.DEFINE_string("data_dir", "/home/jinsoo/SK/doc_cnn", "Provide directory location where glove vectors are unzipped")
...
# Load data
print("Loading data...")
word_index_pickle = open(FLAGS.data_dir + '/sk_train_v1', 'rb')
...
x_train, x_dev = x_shuffled[:-1500], x_shuffled[-1500:]
y_train, y_dev = y_shuffled[:-1500], y_shuffled[-1500:]
...
#                temp = name
        ...
        batches = data_helpers.batch_iter(
            list(zip(x, y)), FLAGS.batch_size, FLAGS.num_epochs, False)
        # Training loop. For each batch...
#        test_finished = pd.DataFrame(columns = ['topic_0','topic_1','topic_2','topic_3','topic_4'])
        test_finished = pd.DataFrame(columns = ['predictions'])
        for batch in batches:
            try :
                x_batch, y_batch = zip(*batch)
#            test_finished.append(pd.DataFrame(test_step(x_batch),columns = ['topic_0','topic_1','topic_2','topic_3','topic_4']))
                data = test_step(x_batch)
#                print (data)
#                print (data[0])
                pd_data = pd.DataFrame(data[0], columns= ['predictions'])
#                print(pd_data)
#                test_finished.append(pd_data, columns = ['predictions'])
                test_finished = pd.concat([test_finished, pd_data])
#                print (test_finished)
            except:
                continue
#        test_finished = pd.DataFrame(test_step(x), columns = ['topic_0','topic_1','topic_2','topic_3','topic_4'])
        print("generating finished file")
...
#        documents = pd.DataFrame(whole_document, columns = ['real_document'])
#        print (documents)
        result = test_finished
#        result = pd.concat([test_finished, documents], axis=1)
        test_file_name = 'final_result.csv'
