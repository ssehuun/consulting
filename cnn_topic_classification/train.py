...
with tf.Graph().as_default():
	...
	# Assigning glove vector to the embedding vector	
        ...
        def test_step(x_batch, writer=None):
            """
            Evaluates model on a dev set
            """
            y_batch = []
            for i in range(len(x_batch)):
				y_batch.append([0,0,0,0,0,0,0,0,0])
            feed_dict = {
              cnn.input_x: x_batch,
              cnn.input_y: y_batch,
              cnn.dropout_keep_prob: 1.0
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                ...
                test_finished = pd.DataFrame(test_step(x_test2, writer=test_summary_writer),columns =['topic2'])
                test_finished.insert(0, 'topic1', test_step(x_test, writer=test_summary_writer))
