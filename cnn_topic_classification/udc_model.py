...
def create_model_fn(hparams, model_impl):
  ...
  def model_fn(features, targets, mode):
    context, context_len = get_id_feature(
        features, "question1", "question1_len", hparams.max_context_len)
#    if mode != tf.contrib.learn.ModeKeys.INFER:
    utterance, utterance_len = get_id_feature(
            features, "question2", "question2_len", hparams.max_utterance_len)

#    if mode == tf.contrib.learn.ModeKeys.EVAL:
#        batch_size = targets.get_shape().as_list()[0]

    if mode == tf.contrib.learn.ModeKeys.TRAIN or mode == tf.contrib.learn.ModeKeys.EVAL:
      probs, loss, _ = model_impl(
          hparams,
          mode,
          context,
          context_len,
